<?php
    ...
    include_once(INCLUDE_DIR. "/UserLogin.php");
    ob_clean();
    ...
    $myUserLogin = new UserLogin($myMySQL);

    $myTemplate = new Template(TEMPLATE_DIR ."/user_login.html");
    ...
    {
        $condition .= " AND login_time >= '$add_time_min'";
    }

    if( !empty($add_time_max) )
    {
        $condition .= " AND login_time <= '$add_time_max 23:59:59'";
    }

    // page
    $page_size = 50;

    $total_count = $myUserLogin->getCount($condition);
    $total_page = $myUserLogin->getPageCount($page_size, $condition);

    $total_page = ($total_page == 0) ? 1 : $total_page;
    $page = ($total_page < $page) ? $total_page : $page;

    // list
    $rows = $myUserLogin->getPage("*", $page, $page_size, $condition ." ORDER BY ". str_replace("-", " ", $order));

    $random = time();

    for($i = 0; isset($rows[$i]); $i++)
    {   
        $dataArray = $myUserLogin->getData($rows[$i]);
