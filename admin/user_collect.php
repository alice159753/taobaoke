<?php
    ...
    $collect_no = !empty($_REQUEST["collect_no"]) ? $_REQUEST["collect_no"] : '';
    ...
    $searchArray['{collect_no}'] = $collect_no;
    ...
    if( !empty($collect_no) )
    {
        $condition .= " AND collect_no = $collect_no";
    }
    ...
    $total_count = $myUserCollect->getCount($condition);
    ...
    // list
    $rows = $myUserCollect->getPage("*", $page, $page_size, $condition ." ORDER BY ". str_replace("-", " ", $order));

    $random = time();

    for($i = 0; isset($rows[$i]); $i++)
    {   
        $dataArray = $myUserCollect->getData($rows[$i]);
