<?php
    ...
    $myTemplate = new Template(TEMPLATE_DIR ."/pay_order.html");
    ...
    $total_count = $myPayOrder->getCount($condition);
    $total_page = $myPayOrder->getPageCount($page_size, $condition);

    $total_page = ($total_page == 0) ? 1 : $total_page;
    $page = ($total_page < $page) ? $total_page : $page;

    // list
    $rows = $myPayOrder->getPage("*", $page, $page_size, $condition ." ORDER BY ". str_replace("-", " ", $order));

    $random = time();

    for($i = 0; isset($rows[$i]); $i++)
    {   
        $dataArray = $myPayOrder->getData($rows[$i]);
