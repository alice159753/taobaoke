<?php
    ...
    $categoryRows = $myCategory->getRows("*", "1=1");

    foreach ($categoryRows as $key => $value) 
    {
        for($i = 1; $i <= 200; $i++)
        {
            $lists = $myTaobaoApi->getPointList($value['title'], $i, 100);
            ...
            foreach ($lists as $key => $item) 
            {
                ...
                haojuanToproduct($myProduct, $item, $value);
            }
        }
    }

    //好卷清单的商品转换为product
    function haojuanToproduct($myProduct, $item = array(), $category)
    {
        $myTaobaoApi = new TaobaoApi();
        ...
        if( !empty($category['parent_no']) )
        {
            $dataArray['category_no']  = $category['no'];
            $dataArray['category_no1'] = $category['parent_no'];
            $dataArray['category_no2'] = $category['no'];
        }
        else
        {
            $dataArray['category_no']  = $category['no'];
            $dataArray['category_no1'] = $category['no'];
            $dataArray['category_no2'] = 0;
        }
