<?php
    ...
    $searchList = $mySearch->getRows("*", "is_finish = 0 ORDER BY no DESC LIMIT 1");

    for($i = 0; isset($searchList[$i]); $i++)
    {
        $dataArray = array();
        $dataArray['is_finish'] = 1;

        $mySearch->update($dataArray, "no = ". $searchList[$i]['no']);

        $search_count = 0;
        for($page = 1; $page <= 2; $page++)
        {
            $lists = $myTaobaoApi->getPointList($searchList[$i]['title'], $page,100);
        ...
        $dataArray = array();
        $dataArray['is_finish'] = 2;
        $dataArray['search_count'] = $search_count;
